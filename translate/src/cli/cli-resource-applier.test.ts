...
describe("apply_cli_resources", () => {
  ...
  it("disables implicit quality and prompt resources when no explicit resource is passed", async () => {
    ...
    expect(read_meta(services.operations, "translation_prompt_enable")).toBe(false);
    expect(services.operations.some((operation) => operation.name === "setRules")).toBe(false);
  });

  it("imports explicit prompt text and enables only the translation prompt", async () => {
    ...
    expect(
      services.operations.find(
        (operation) =>
          ...
          operation.args?.["ruleType"] === "translation_prompt",
      )?.args?.["text"],
    ).toBe("custom prompt");
    expect(read_meta(services.operations, "translation_prompt_enable")).toBe(true);
    expect(read_meta(services.operations, "quality_prompt_revision.translation")).toBe(1);
    expect(
      services.operations
        .filter((operation) => operation.name === "setRuleText")
        .map((operation) => operation.args?.["ruleType"]),
    ).toEqual(["translation_prompt"]);
  });

  function create_command(
    resources: Partial<TranslateCliOptions["resources"]> = {},
  ): TranslateCliOptions {
    return {
      mode: "new",
