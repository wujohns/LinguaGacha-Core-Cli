...
const REC_MODEL_DIR = path.join(SCRIPT_DIR, "model", "tiny-rec");
...
async function main() {
  ...
  const [comicModelConfig, comicPreprocessorConfig, detConfig, recConfig] = await Promise.all([
    ...
    assertFile(REC_MODEL_ONNX_PATH, "make tiny-rec"),
  ]);
...
async function runImage({
  ...
  const outputJsonPath = path.join(ASSETS_DIR, `${imageBaseName}.comic-bubble-medium-det-tiny-rec.json`);
  const debugImagePath = path.join(ASSETS_DIR, `${imageBaseName}.comic-bubble-medium-det-tiny-rec.png`);
  const cropDir = path.join(ASSETS_DIR, `${imageBaseName}.comic-bubble-medium-det-tiny-rec-crops`);
