...
export type ProjectPrefilterWriteOutput = {
  ...
  translation_extras: Record<string, unknown>; // 按最终 item 状态重建的项目进度 meta，字段名兼容 .lg 摘要
  project_settings: {
...
export type ProjectPrefilterWriteInput = {
  ...
  task_snapshot?: Record<string, unknown>; // 可选旧进度基底，缺省时从空分析进度开始
 ...
 * 构建当前场景的稳定结果。
 */
function build_progress_extras(task_snapshot: Record<string, unknown>): Record<string, unknown> {
  ...
  const progress_extras: Record<string, unknown> = {};
  for (const [key, value] of Object.entries(task_snapshot)) {
    ...
    progress_extras[key] = value;
  }
  return progress_extras;
}

// 构造空闲分析任务快照，供后端 reset 或无历史进度时作为统计基底。
/**
 * 构建当前场景的稳定结果。
 */
export function create_empty_analysis_task_snapshot(): Record<string, unknown> {
  return {
    task_type: "analysis",
    status: "idle",
    busy: false,
    ...
    extras: { kind: "analysis", scope: { kind: "all" } },
  };
}

// 按最终 item 状态重建项目进度 meta，task snapshot 运行态由专属任务模块发布。
/**
 * 构建当前场景的稳定结果。
 */
export function build_progress_extras_from_items(args: {
  ...
  const progress_extras = build_progress_extras(args.task_snapshot);
  progress_extras.processed_line = processed_line;
  progress_extras.error_line = error_line;
  progress_extras.total_line = total_line;
  progress_extras.line = processed_line + error_line;
...
export function compute_project_prefilter_write(
  ...
  const progress_extras = build_progress_extras_from_items({
    task_snapshot: input.task_snapshot ?? create_empty_analysis_task_snapshot(),
    items: item_index,
  });

  return {
    items: next_items,
    translation_extras: progress_extras,
    project_settings: {
...
type RevisionBackedSection = "files" | "items" | "analysis";
