...
export class ProjectDefaultPresetInitializer {
  ...
  public build_operations(project_path: string): ProjectDefaultPresetInitializationResult {
    ...
    const analysis_prompt = Prompt.analysis();
    const prompt_virtual_id = this.string_value(config[analysis_prompt.default_preset_setting_key]);
    if (prompt_virtual_id !== "") {
      try {
        const text = this.read_prompt_preset(analysis_prompt, prompt_virtual_id);
        operations.push(...this.build_prompt_operations(project_path, analysis_prompt, text));
        loaded_names.push("分析提示词");
      } catch (error) {
        this.log_non_blocking_warning(
          t_main_log("app.diagnostic.default_preset.prompt_load_failed"),
          error,
          { task_type: analysis_prompt.kind, virtual_id: prompt_virtual_id },
        );
      }
    }
  ...
  private read_prompt_preset(prompt: ReturnType<typeof Prompt.analysis>, virtual_id: string): string {
  ...
  private resolve_prompt_preset_path(
    prompt: ReturnType<typeof Prompt.analysis>,
    virtual_id: string,
  ): string {
