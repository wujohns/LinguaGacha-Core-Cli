...
import { AnalysisCandidateExportService } from "../backend/analysis/analysis-candidate-export-service";
...
import type { AnalyzeCliLimiterOptions } from "./cli-parser";

export interface AnalyzeRuntimeStartOptions {
  appRoot: string;
  configPath: string;
  workerExecution?: BackendWorkerExecution;
  workerCount?: number;
  limiter?: AnalyzeCliLimiterOptions | null;
}

export class AnalyzeRuntime {
  ...
  private readonly limiter: AnalyzeCliLimiterOptions | null;
  ...
  private services: AnalyzeRuntimeServices | null = null;

  public constructor(options: AnalyzeRuntimeStartOptions) {
    ...
    this.data_root = path.join(path.dirname(this.config_path), ".linguagacha-analyze-runtime");
  }

  public async start(): Promise<AnalyzeRuntimeServices> {
    ...
    const cache_manager = new AnalyzeCache(database);
    ...
    const analysis_candidate_export_service = new AnalysisCandidateExportService(
      database,
      project_session_state,
    );
    ...
    this.services = new AnalyzeRuntimeServices({
      appSettingService: app_setting_service,
      database,
      ...
      analysisCandidateExportService: analysis_candidate_export_service,
...
export class AnalyzeRuntimeServices {
  ...
  public readonly analysisCandidateExportService: AnalysisCandidateExportService;
  ...
  public constructor(options: {
    ...
    analysisCandidateExportService: AnalysisCandidateExportService;
    ...
    this.analysisCandidateExportService = options.analysisCandidateExportService;
...
class AnalyzeCache implements CacheReadPort {
