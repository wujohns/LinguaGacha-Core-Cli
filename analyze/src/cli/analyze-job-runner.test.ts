...
class FakeServices {
  ...
  public readonly analysisCandidateExportService = {
    export_analysis_candidates_to_directory: async (output_dir: string) => {
...
describe("run_analyze_job", () => {
  ...
  it("rejects mode new when the .lg project already exists", async () => {
    ...
    await expect(
      run_analyze_job(services.as_runtime_services(), command, {
  ...
  it("creates, runs, exports, and unloads a new project", async () => {
    ...
    await run_analyze_job(services.as_runtime_services(), command, {
      statusReporter: new CLIJsonStatusReporter({ writeLine: (line) => lines.push(line) }),
    });

    expect(services.created_projects).toHaveLength(1);
    expect(services.loaded_projects).toEqual([]);
    expect(services.restore_failed_count).toBe(0);
  ...
  function create_fixture(mode: AnalyzeCliOptions["mode"]): { command: AnalyzeCliOptions } {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "linguagacha-analyze-cli-job-"));
    temp_dirs.push(root);
