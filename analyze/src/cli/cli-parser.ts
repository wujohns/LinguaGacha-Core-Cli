...
export type AnalyzeCliMode = "new" | "continue" | "reset";

export type AnalyzeCliParseResult =
  | { kind: "help" }
  | { kind: "version" }
  | { kind: "command"; command: AnalyzeCliOptions };
...
export interface AnalyzeCliOptions {
  ...
  resources: AnalyzeCliResources;
}

export interface AnalyzeCliLimiterOptions {
  url: string;
  resource: string;
}

export interface AnalyzeCliResources {
...
const MODES = new Set<AnalyzeCliMode>(["new", "continue", "reset"]);
...
export function parse_cli_args(argv: string[]): AnalyzeCliParseResult {
...
function parse_command_options(tokens: string[]): AnalyzeCliOptions {
  let mode = "";
  let project_path = "";
  let config_path = "";
  let output_dir = "";
...
function create_empty_resources(): AnalyzeCliResources {
  return {
    promptPath: null,
  };
}

function normalize_mode(value: string): AnalyzeCliMode {
  const mode = value.trim().toLowerCase();
  if (MODES.has(mode as AnalyzeCliMode)) {
    return mode as AnalyzeCliMode;
  }
